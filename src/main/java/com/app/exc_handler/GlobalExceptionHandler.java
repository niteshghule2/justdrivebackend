...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<?> handleRuntimeException(RuntimeException e) {
		...
		ErrorResponse resp = new ErrorResponse("failed...!!!!!", LocalDateTime.now());
	 ...
	 * ResponseEntity<?> handleEmptyDataExcetpion(EmptyResultDataAccessException e)
	 * { System.out.println("in handle  exc " + e); ErrorResponse resp = new
	 * ErrorResponse(e.getMessage(), LocalDateTime.now()); return
	 * ResponseEntity.status(HttpStatus.NOT_FOUND).body(resp); }
	 * 
	 * @ExceptionHandler(UserHandlingException.class) public ResponseEntity<?>
	 * handleUserHandlingException(UserHandlingException e) {
	 * System.out.println("in handle user exc " + e); ErrorResponse resp = new
	 * ErrorResponse(e.getMessage(), LocalDateTime.now()); return
	 * ResponseEntity.status(HttpStatus.NOT_FOUND).body(resp); }
	 * 
	 ...
	 * System.out.println(ex.getBindingResult().getFieldErrors()); StringBuilder sb
	 * = new StringBuilder("Validation Errors : ");
	 * ex.getBindingResult().getFieldErrors().forEach(e ->
	 * sb.append(e.getDefaultMessage()+" ")); return
	 * ResponseEntity.status(HttpStatus.BAD_REQUEST) .body(new
	 * ErrorResponse(sb.toString(), LocalDateTime.now())); }
	 */
